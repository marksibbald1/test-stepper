...
/// The state of a [VivanStep] which is used to control the style of the circle and
/// text.
///
/// See also:
///
...
/// Defines the [VivanStepper]'s main axis.
enum VivanStepperType {
...
///
/// Used by [VivanStepper.controlsBuilder].
@immutable
class VivanControlsDetails {
  /// Creates a set of details describing the Stepper.
  const VivanControlsDetails({
  ...
  /// just activated the transition to a new step, some [VivanStepper.type] values will
...
@immutable
class VivanStep {
  /// Creates a step for a [VivanStepper].
  ///
  ...
  const VivanStep({
    ...
    this.state = VivanStepState.indexed,
    this.isActive = false,
    this.label,
  });
  ...
  /// Only [VivanStepperType.horizontal], Optional widget that appears under the [title].
...
///
/// {@tool dartpad}
/// An example the shows how to use the [VivanStepper], and the [VivanStepper] UI
/// appearance.
///
...
///  * [VivanStep]
///  * <https://material.io/archive/guidelines/components/steppers.html>
class VivanStepper extends StatefulWidget {
  ...
  const VivanStepper({
    ...
    required this.steps,
    this.physics,
    this.type = VivanStepperType.vertical,
    this.currentStep = 0,
    this.onStepTapped,
    this.onStepContinue,
    this.onStepCancel,
  ...
  /// [VivanStepperType.horizontal], the content of the current step is displayed
  /// underneath as opposed to the [VivanStepperType.vertical] case where it is
  /// displayed in-between.
  final VivanStepperType type;
  ...
  ///
  /// This callback which takes in a context and a [VivanControlsDetails] object, which
  ...
  /// [VivanControlsDetails.currentStep] value within the callback can change the text
  ...
  /// {@end-tool}
  final VivanControlsWidgetBuilder? controlsBuilder;

  /// The elevation of this stepper's [Material] when [type] is [VivanStepperType.horizontal].
  ...
  ///
  /// When overriding icon for [VivanStepState.error], please return
  /// a widget whose width and height are 14 pixels or less to avoid overflow.
  ///
  /// If null, the default icons will be used for respective [VivanStepState].
  final VivanStepIconBuilder? stepIconBuilder;

  @override
  State<VivanStepper> createState() => _StepperState();
}

class _StepperState extends State<VivanStepper> with TickerProviderStateMixin {
  late List<GlobalKey> _keys;
  final Map<int, VivanStepState> _oldStates = <int, VivanStepState>{};
  ...
  @override
  void didUpdateWidget(VivanStepper oldWidget) {
  ...
  bool _isLabel() {
    for (final VivanStep step in widget.steps) {
      if (step.label != null) {
        return true;
      }
    }
  ...
  Widget _buildCircleChild(int index, bool oldState) {
    ...
    switch (state) {
      ...
      case VivanStepState.complete:
        return Icon(
          Icons.check,
          color: isDarkActive ? _kCircleActiveDark : _kCircleActiveLight,
          size: 18.0,
        );
      case VivanStepState.error:
  ...
  Widget _buildIcon(int index) {
    if (widget.steps[index].state != _oldStates[index]) {
      ...
      if (widget.steps[index].state != VivanStepState.error) {
  ...
  Widget build(BuildContext context) {
    ...
    assert(() {
      if (context.findAncestorWidgetOfExactType<VivanStepper>() != null) {
        throw FlutterError(
    ...
    switch (widget.type) {
      case VivanStepperType.vertical:
        return _buildVertical();
      case VivanStepperType.horizontal:
